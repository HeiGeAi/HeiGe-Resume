...
![Version](https://img.shields.io/badge/version-2.3.2-ff4d12.svg)
...
越来越多招聘流程会先经过 ATS、关键词检索或 AI 辅助排序，但采用率和权重因公司、岗位与地区而异，本项目不把单一比例当行业事实。你的简历需要先被机器正确解析，再让招聘者快速看见最强证据。大多数 AI 做出来的简历是一张流水账：通篇负责什么、参与什么，没有一个数字，再套个跟八百份一样的双栏模板，两关都过不去。
...
- ✅ 零 JS 依赖单文件（字体走 Google Fonts，带完整中文系统字体兜底，字体加载失败也正常显示），到处都能用
...
完整对比 + 逐条拆解见 **[在线 Demo](https://heigeai.github.io/HeiGe-Resume/examples/comparison.html)**，测试方法见 [`tools/ai-screener-eval.md`](tools/ai-screener-eval.md)。这是一次使用自定义 rubric 的本地合成评测，不是行业基准；仓库保留输入、方法和汇总分，但没有保留六次模型响应的逐字原始 JSON，因此这些数字只能说明本次实验方向，不能外推成通用提升幅度。
...
把岗位的关键词挑出来、用它的原话复述、最相关的挪到最前。本项目的本地 rubric 把岗位匹配设为最高权重之一，用于检验内容是否对岗；这不是招聘平台公开公式，也不承诺固定回复率提升。
...
同一套方法论，做出八份样例。每份都是现场锻造的零 JS 依赖单文件（字体走 Google Fonts，带中文系统字体兜底），**A4 排版、文字可选中、Cmd+P 完美导出 PDF**。全部为虚构人物，保护隐私。点开任意预览图即可看在线 Demo。
...
Claude Code 会自动读取 `~/.claude/skills/` 目录：

```bash
git clone https://github.com/HeiGeAi/HeiGe-Resume.git ~/.claude/skills/heige-resume
```
...
### 方法 2：作为自定义指令 / System Prompt 使用

适用于 Cursor、Windsurf、Cline、ChatGPT、Claude.ai、Aider、OpenClaw、Hermes 等不读取 `~/.claude/skills/` 目录的平台：

1. 下载本仓库的 `SKILL.md`
2. 将内容粘贴到该平台的自定义指令 / 规则 / system prompt 中（如 Cursor Rules、Windsurf Rules、Cline 的 Custom Instructions）
3. 直接对话使用，无需特殊命令
...
**离线投递**：成品默认从 Google Fonts 拉字体，带完整中文系统字体兜底，加载失败也正常显示。投递环境完全不能联网、或不想打开文件就向第三方发请求时，删掉 HTML 头部 fonts.googleapis.com 的两个 `<link>` 标签即可，版式由系统字体接管。
...
| 平台 | 能不能用 | 安装方式 | 输出怎么拿 |
|------|:---:|---------|-----------|
| **Claude Code** | ✅ | 克隆到 `~/.claude/skills/heige-resume` | 自动写成 .html |
| **Cursor / Windsurf / Cline** | ✅ | 把 SKILL.md 贴进各家自定义规则（Cursor Rules / Windsurf Rules / Cline Custom Instructions） | 自动写成 .html |
...
### v2.3.3 (2026-09-13)
- 🛠 可编辑层修复：存档不再带入编辑态属性（刷新后不会误进可编辑状态）、保存失败如实提示、模板指纹防止旧改动覆盖新模板、恢复存档前消毒 innerHTML
- ✅ 新增编辑层行为回归与副本一致性测试，CI 的 npm audit 收敛到 high 级别
- 📝 README 补「离线投递」说明（删掉 Google Fonts link 即纯系统字体版）

### v2.3.2 (2026-07-31)
- ♿ 7 份简历样例补齐唯一 `h1` 主标题，同时保持原有视觉尺寸和打印布局
- 🔎 把 AI 筛选权重、采用率和回复提升改为明确的内部 rubric 与方向性实验边界，不再把单次数据写成行业事实
- ✅ 新增样例可靠性测试、CI 与私密安全报告入口

### v2.3.1 (2026-06-10)
- 🧹 占位符与文档修复：示例 GitHub 句柄改为 `your_username`（下划线在 GitHub 用户名里不合法，永不指向真人）、虚构作品集域名改用保留 TLD `.example`、虚构开源项目改名 `go-ratesluice` 避免与真实仓库撞名，预览图同步重制
- 📝 文档一致性：SKILL.md frontmatter 合规化、安装命令落地目录与 skill name 对齐、平台安装方式按各家实际行为改写、工具栏位置文档统一为左下角

### v2.3.0 (2026-06-08)
- ✏️ 可编辑层：每份简历打开就能在浏览器里直接改文字（点左下角「编辑」），改完导出 PDF 或下载一份带改动的独立 HTML，改动自动存本地、刷新不丢
