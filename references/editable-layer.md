...
(function(){
  ...
  var editing = false, saveTimer = null, stale = false;
  function hash(s){ var h = 5381; for (var i = 0; i < s.length; i++){ h = ((h << 5) + h + s.charCodeAt(i)) >>> 0; } return h.toString(36); }
  var FP = hash(roots.map(function(r){ return r.innerHTML; }).join('\n'));
  ...
  function save(){
    try {
      localStorage.setItem(KEY, JSON.stringify({ v: FP, d: roots.map(function(r){
        var c = r.cloneNode(true);
        [].slice.call(c.querySelectorAll('[contenteditable]')).forEach(function(n){ n.removeAttribute('contenteditable'); n.removeAttribute('spellcheck'); });
        return c.innerHTML;
      }) }));
      status('已保存');
    } catch(e){ status('保存失败：浏览器禁止本地存储或空间已满'); }
  }
  function sanitize(h){
    var t = document.createElement('template');
    t.innerHTML = h;
    [].slice.call(t.content.querySelectorAll('script')).forEach(function(n){ n.remove(); });
    [].slice.call(t.content.querySelectorAll('*')).forEach(function(n){
      [].slice.call(n.attributes).forEach(function(a){ if (/^on/i.test(a.name)) n.removeAttribute(a.name); });
    });
    return t.innerHTML;
  }
  function restore(){
    try {
      ...
      if (data.v !== FP || !Array.isArray(data.d) || data.d.length !== roots.length){
        stale = true; try { localStorage.removeItem(KEY); } catch(e2){}
        return false;
      }
      data.d.forEach(function(h,i){ roots[i].innerHTML = sanitize(h); });
      return true;
    } catch(e){}
    return false;
  }
